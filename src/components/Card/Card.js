...
function Card(props) {
    ...
    const [isAddedCard, setIsAddedCart] = useState(Boolean);

    useEffect(() => {
        props.addTocard({isAddedCard, imgUrl, title, price});
    }, [isAddedCard,
        !isAddedCard])

    const onClickAddCard = () => {
        setIsAddedCart(!isAddedCard);
    }

    return ( 
        <div className={`${styles.card} mb-20`}>
            <button onClick={props.onClickAddFavorite} className={'btn' + ' ' + styles.btnAddFavorite}>
            ...
            <img className={styles.sneakersImgCart} src={imgUrl} alt={title}/>
            <p>{title}</p>
            <div className={`${styles.infoCardTotal} d-flex justify-between`}>
                <div>
                <p>Цена:</p>
                <h5>{price} руб.</h5>
                </div>
                {!isAddedCard ? <ButtonSecondary onClick={onClickAddCard} clazz={`btn`} srcImg="/img/addCart.svg" /> :
                <ButtonSecondary onClick={onClickAddCard} clazz={`btn ${styles.activeAddCard}`} srcImg="/img/checked.svg" />}
            </div>
        </div>    
    );
}
